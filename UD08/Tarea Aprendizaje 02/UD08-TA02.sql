...
BEGIN
    EXECUTE IMMEDIATE 'DROP VIEW emp_details_view';
...
/*
Consulta 1: Bloque anónimo para crear un array asociativo con asignaturas 
y visualizarlas mediante un bucle FOR.
*/
SET SERVEROUTPUT ON;
DECLARE
...
BEGIN
    ...
    datos(4) := 'LITERATURA';
    FOR i IN 1..4 LOOP
        dbms_output.put_line('Clave ' || i || ' Valor: ' || datos(i));
    END LOOP;
END;
/

/* 
Consulta 2: Procedimiento que carga empleados de un departamento en un array 
y los visualiza.
*/
CREATE OR REPLACE PROCEDURE array2(codigo NUMBER) IS
    TYPE emp IS TABLE OF employees%ROWTYPE INDEX BY PLS_INTEGER;
    tabla emp;
    indice NUMBER:=0;
    CURSOR datos2 IS
        SELECT *
...
BEGIN
    FOR i IN datos2 LOOP
        ...
        tabla(indice) := i;
    END LOOP;
    FOR x IN 1..indice LOOP
        dbms_output.put_line('Empleado' || x || ': ' || tabla(x).first_name || ' ' || tabla(x).last_name);
    END LOOP;
END;
/

/*
Consulta 3: Procedimiento prueba_array1 v2 usando %ROWCOUNT 
y métodos FIRST/LAST.
*/
CREATE OR REPLACE PROCEDURE array3(codigo3 NUMBER) IS
    TYPE emp3 IS TABLE OF employees%ROWTYPE INDEX BY PLS_INTEGER;
    tabla3 emp3;
    CURSOR datos3 IS
        SELECT *
        FROM employees
        WHERE department_id = codigo3;
BEGIN
    FOR i IN datos3 LOOP
        tabla3(datos3%rowcount) := i;
    END LOOP;
    FOR x IN tabla3.first..tabla3.last LOOP
        dbms_output.put_line('Empleado' || x || ': ' || tabla3(x).first_name || ' ' || tabla3(x).last_name);
    END LOOP;
END;
/

/*
Consulta 4: Procedimiento prueba_array2 usando BULK COLLECT 
y el método COUNT.
*/
CREATE OR REPLACE PROCEDURE array4(codigo4 NUMBER) IS
    TYPE emp4 IS TABLE OF employees%ROWTYPE INDEX BY PLS_INTEGER;
    tabla4 emp4;
BEGIN
    SELECT * BULK COLLECT INTO tabla4 FROM employees WHERE department_id = codigo4;
    FOR i IN 1..tabla4.COUNT LOOP
        dbms_output.put_line('Empleado' || i || ': ' || tabla4(i).first_name || ' ' || tabla4(i).last_name);
    END LOOP;
END;
/

/*
Consulta 5: Modificación de prueba_array2 para manipulación avanzada.
*/
CREATE OR REPLACE PROCEDURE array5(codigo5 NUMBER) IS
    TYPE emp5 IS TABLE OF employees%ROWTYPE INDEX BY PLS_INTEGER;
    tabla5 emp5;
    indice5 NUMBER;
BEGIN
    SELECT * BULK COLLECT INTO tabla5 FROM employees WHERE department_id = codigo5;
    FOR i IN 1..tabla5.COUNT LOOP
        dbms_output.put_line('Empleado' || i || ': ' || tabla5(i).first_name || ' ' || tabla5(i).last_name);
    END LOOP;
    
    dbms_output.put_line('PRIMER EMPLEADO: ' || tabla5(tabla5.FIRST).first_name || ' ' || tabla5(tabla5.FIRST).last_name);
    dbms_output.put_line('SEGUNDO EMPLEADO: ' || tabla5(tabla5.LAST).first_name || ' ' || tabla5(tabla5.LAST).last_name);
    
    IF tabla5.EXISTS(200) THEN
        dbms_output.put_line('El empleado 200 existe');
    ELSE
        dbms_output.put_line('El empleado 200  NO existe');
    END IF;
    dbms_output.put_line('ANTES DEL BORRADO HAY:'||tabla5.COUNT);
    
    indice5 := tabla5.FIRST;
    WHILE indice5 IS NOT NULL LOOP
        IF tabla5(indice5).salary > 5000 THEN
            tabla5.DELETE(indice5);
        END IF;
        indice5 := tabla5.NEXT(indice5);
    END LOOP;
    dbms_output.put_line('DESPUES DEL BORRADO HAY:'||tabla5.COUNT);
    
    indice5 := tabla5.FIRST;
    WHILE indice5 IS NOT NULL LOOP
        dbms_output.put_line(tabla5(indice5).first_name||' '||tabla5(indice5).last_name);
        indice5 := tabla5.NEXT(indice5);
    END LOOP;
END;
/
