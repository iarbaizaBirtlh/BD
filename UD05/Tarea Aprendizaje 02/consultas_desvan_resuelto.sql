/*Solución de la tarea de aprendizaje 2 para BD05.
Estas son las respuestas a las consultas que se preguntan en la tarea. Recuerda que algunas consultas no tienen una única forma de resolverse*/
...
FROM empleados INNER JOIN trabajan 
  ...
  INNER JOIN departamentos 
    ON departamentos.codigo = trabajan.cod_dep 
WHERE empleados.hijos = 1;

--14.Nombre del empleado y nombre del departamento en el que han trabajado los empleados que no tienen hijos.
...
FROM empleados INNER JOIN just_nominas 
  ON empleados.codigo = just_nominas.cod_emp 
  INNER JOIN lineas 
    ON just_nominas.cod_emp = lineas.cod_emp AND just_nominas.mes = lineas.mes AND just_nominas.ejercicio = lineas.ejercicio 
...
FROM empleados INNER JOIN just_nominas 
  ...
  INNER JOIN trabajan 
    ON empleados.codigo = trabajan.cod_emp 
    INNER JOIN departamentos ON departamentos.codigo = trabajan.cod_dep 
WHERE departamentos.nombre = 'Ventas';

--17.Nombre del empleado e ingresos totales percibidos agrupados por nombre.
SELECT empleados.nombre, SUM(lineas.cantidad) 
FROM empleados INNER JOIN just_nominas 
  ON empleados.codigo = just_nominas.cod_emp 
  INNER JOIN lineas 
    ON just_nominas.cod_emp = lineas.cod_emp AND just_nominas.mes = lineas.mes AND just_nominas.ejercicio = lineas.ejercicio 
...
--Opción1
SELECT empleados.nombre 
FROM empleados LEFT OUTER JOIN just_nominas 
  ON empleados.codigo = just_nominas.cod_emp 
...
SELECT nombre, to_char(fnacimiento, 'DD/MM/YYYY') AS fecha_nacimiento 
FROM empleados;

--23.Nombre y fecha de nacimiento con formato "1 de Enero de 2000" y etiquetada la columna como fecha, de todos los empleados.
SELECT nombre, to_char(fnacimiento, 'DD')||' de '||to_char(fnacimiento, 'Month')||' de '||to_char(fnacimiento, 'YYYY') AS fecha 
FROM empleados;
...
SELECT nombre, TRUNC(months_between(sysdate, fnacimiento) / 12), hijos 
FROM empleados 
...
SELECT empleados.nombre, TRUNC(months_between(sysdate, empleados.fnacimiento) / 12), departamentos.nombre 
FROM empleados INNER JOIN trabajan
  ON empleados.codigo = trabajan.cod_emp 
  INNER JOIN departamentos
  ON  departamentos.codigo = trabajan.cod_dep 
...
SELECT nombre, TRUNC(months_between(sysdate, fnacimiento) / 12), cuenta 
FROM empleados 
WHERE MOD(TRUNC(months_between(sysdate, fnacimiento) / 12), 3) = 0;

--30.Nombre e ingresos percibidos del empleado más joven y del más longevo.
SELECT empleados.nombre, SUM(just_nominas.ingreso)-SUM (just_nominas.descuento) 
FROM empleados INNER JOIN just_nominas
  ON empleados.codigo = just_nominas.cod_emp 
  WHERE(empleados.fnacimiento = (SELECT MIN(fnacimiento) FROM empleados) OR empleados.fnacimiento = (SELECT MAX(fnacimiento) FROM empleados)) 
GROUP BY empleados.nombre;